...
} satisfies Record<ManageKind, { path: string; exportName: string; prefix: string; commitScope: string }>;
...
async function getExistingFile(filePath: string) {
  const url = new URL(`https://api.github.com/repos/${githubConfig.owner}/${githubConfig.repo}/contents/${filePath}`);
  ...
  if (!encoded || typeof data?.sha !== "string") throw new Error(`GitHub file payload is invalid: ${filePath}`);
...
async function commitFile(filePath: string, sha: string, content: string, message: string) {
...
export const POST: APIRoute = async ({ cookies, request }) => {
  ...
  try {
    ...
    return json({ error: error instanceof Error ? error.message : "Manage update failed." }, 400);
  }
};
