...
function frontmatter(lines: Array<[string, string | boolean | string[] | undefined]>) {
  const content = lines
...
function buildMarkdown(payload: PublishPayload) {
  ...
  if (type !== "blog" && type !== "note") throw new Error("Invalid publish type.");
...
async function publishProject(payload: PublishPayload) {
  ...
  const nextContent = current.content.replace(/\r?\n\];\s*$/, `\n${nextProject}\n];\n`);

  if (nextContent === current.content) {
    throw new Error("Unable to locate projects array ending in src/data/projects.ts.");
  }
